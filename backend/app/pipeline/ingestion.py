...
from app.pipeline import chunking, embeddings, extraction, node_extraction
...
async def run_ingestion_pipeline(doc_id: str, text: str, job_id: str) -> None:
    ...
    driver = get_driver()
    async with driver.session() as session, driver.session() as node_session:
        if chunks:
        ...
        for chunk in chunks:
            fact_count, _node_count = await asyncio.gather(
                process_chunk_extraction(session, chunk, doc_id, job_id),
                process_chunk_node_extraction(node_session, chunk, doc_id, job_id),
            )
