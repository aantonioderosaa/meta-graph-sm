...
- Giudice (Fase 10): a fine di ogni batch di dreaming (dopo `reconcile`) gira `run_judge` — anti-blur, `EQUIVALENT_TO`, ri-raffinamento storico, conferma identità, `CONTRADICTS` mancate, smistamento temporale. Scrive solo primitive esistenti (INGEST/PROMOTE + Famiglia B / `MEMBER_OF`). Ogni passata è loggata in `:JudgeRun`. Flag `ENABLE_JUDGE` (default true); `BACKBONE_COLLAPSE_THRESHOLD=0.90`. Dalla Fase 18 il compito 5 (`FIND_MISSED_CONTRADICTIONS_CYPHER`) è scoped a `touched_ids` del batch; lista vuota/assente resta full-scan.
...
- Event triage (gated, default off): `_task_event_triage` in `run_judge` (ultimo task) gira un loop ReAct dedicato (`EventTriageStep`, cap `EVENT_TRIAGE_MAX_TURNS=3`; turno 0 gratis = `get_relations`/`get_metadata` + testo grezzo `:Chunk` via `DERIVED_FROM`, cap 4000). `propose` è l'azione terminale; `search_fulltext`/`search_vector` prima di arrenderti se un'entità nominata manca dalle osservazioni. Il giudice non crea mai nuovi `:Node` — assert/retract solo su id già osservati (gate) e già esistenti (MATCH). Scrive solo tramite `validate_slot_proposal` / `apply_validated_slot`. `apply_validated_slot` ritorna False se `head_id`/`tail_id` non esistono come `:Node` o se retract è un no-op — un id allucinato non può più produrre `confirmed` senza scrittura. `verified_no_change=True` (lista slot vuota) conferma senza scrittura e senza consumare un check della finestra di attesa; lista vuota senza il flag resta `waiting`/`incomplete`. Audit `:EventTriageRun` keyed per evento. Flag `ENABLE_EVENT_TRIAGE` default **false**, indipendente da `ENABLE_CONTEXT_LAYER` — a flag spento Fase 0–22 identico, zero query extra. Macrotask 8–9 (suite `tests/test_acceptance_event_triage.py`) è verde, incluso il caso e2e in cui l'entità target non è pre-collegata all'evento e viene risolta via `search_fulltext`; il flag resta False fino a un rollout di produzione esplicito. UI: tab **Visualizza incompletezze** legge `GET /graph/event-incompleteness` (nessuna scrittura).
...
- Layer Metagraph in UI (Fase 12): tab laterali Dominio / Identità / Contraddizioni / Regole / Giudice / **Visualizza incompletezze** (liste e albero, nessun canvas NVL extra). Badge ` · faccette` sui nodi multi-identità; citazioni query ASSERITO/DERIVATO. Incompletezze: `GET /graph/event-incompleteness` (sola lettura, elenco `:EventTriageRun` con `verdict=incomplete`; il flag `ENABLE_EVENT_TRIAGE` non è richiesto per listare).
...
Policy di rollout: ogni flag resta spento finché la relativa suite di accettazione non è verde. Fasi 4–12 sono verdi salvo le faccette; `ENABLE_FACET_IDENTITY` e `ENABLE_CONTEXT_LAYER` restano False. Macrotask 8 è verde; `ENABLE_EVENT_TRIAGE` resta False fino a un rollout di produzione esplicito (stessa policy di `ENABLE_CONTEXT_LAYER`).
...
| `ENABLE_EVENT_TRIAGE` | **False** | giudice assert/retract su `:Evento` (Macrotask 8 verde; spento fino a rollout esplicito; indipendente da `ENABLE_CONTEXT_LAYER`) |
| `CONTEXT_AGENT_MAX_TURNS` | 4 | Fase 22: cap turni ReAct (non ritoccare) |
| `EVENT_TRIAGE_MAX_TURNS` | 3 | Macrotask 9: cap turni ReAct del giudice eventi (turno 0 gratis; non ritoccare `CONTEXT_AGENT_MAX_TURNS`) |
...
**Fasi 23–25 — regressione, non riprendere.** Implementate e poi revertite (`3afa8f2` / `ca3071d` / `c0327e8` su `0173c1f`+`67363b4`+`370216a`). Su ingestione live della favola *Il Sole e il Vento* (KB vuota) il codice 23–25 produceva un grafo **povero**: `merge_nodes` fondeva cross-categoria (Sole→prova, Vento→forza, mantello→viandante). Lo stesso protocollo sul codice Fase 22 dà un grafo **buono** (i quattro referenti restano distinti). Il prompt F23.6 estraeva `strada` ma al prezzo di quelle fusioni. Non reintrodurre istanze generiche, harness/fallback F24 né osservabilità F25 finché la risoluzione identità non è protetta da un test live di non-regressione su questa storia.
...
| 22 | Orchestrazione: il flusso agentico di verifica | completata (**HEAD di produzione**) |
| 23 | Istanze generiche per sottodominio | **sbagliata — revertita** (regressione `merge_nodes`) |
| 24 | Validazione layer agentico di contesto | **sbagliata — revertita** |
| 25 | Messa in produzione controllata del layer | **sbagliata — revertita** |
