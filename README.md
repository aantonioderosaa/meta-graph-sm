...
- Event triage (gated, default off): `_task_event_triage` in `run_judge` (ultimo task) propone slot via `call_structured` e scrive solo tramite `validate_slot_proposal` / `apply_validated_slot`. `apply_validated_slot` ritorna False se `head_id`/`tail_id` non esistono come `:Node` o se retract è un no-op — un id allucinato non può più produrre `confirmed` senza scrittura. Audit `:EventTriageRun` keyed per evento. Flag `ENABLE_EVENT_TRIAGE` default **false**, indipendente da `ENABLE_CONTEXT_LAYER` — a flag spento Fase 0–22 identico, zero query extra. Macrotask 8 (suite `tests/test_acceptance_event_triage.py`) è verde; il flag resta False fino a un rollout di produzione esplicito. UI: tab **Visualizza incompletezze** legge `GET /graph/event-incompleteness` (nessuna scrittura).
