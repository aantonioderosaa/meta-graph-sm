...
| `ENABLE_CONTEXT_LAYER` | **False** | Fase 20–24 (spento → comportamento Fase 0–18 identico) |
...
- **Eval layer di contesto (Fase 24)**: da `backend/`, `python scripts/eval_context_layer.py --gate-only` (zero OpenAI) oppure `--with-model-fallback` (stub a cue se manca la chiave; `--live-llm` per il modello reale). Stampa precisione/recall misurati sul corpus F24.1.
