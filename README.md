...
- Identità per faccette (Fase 8): `link_as_facet` / `unlink_facet` legano `:Node` a `:IdentityNode` con `SAME_AS` senza spostare né cancellare `:Relation` / `HAS_CONCEPT` / `DERIVED_FROM`. Ingestione e dreaming, a flag acceso, scrivono solo `POSSIBLY_SAME_AS` (la conferma `SAME_AS` è del giudice, Fase 10). `ENABLE_FACET_IDENTITY` default **false** — il percorso attuale resta `merge_nodes`. Blocking: stessa `kernel_category` e (nome normalizzato / alias / cosine ≥ `IDENTITY_BLOCK_THRESHOLD=0.82`); `NOT_SAME_AS` esclude la coppia.
...
- Giudice (Fase 10 + 23): a fine di ogni batch di dreaming (dopo `reconcile`) gira `run_judge` — anti-blur, `EQUIVALENT_TO`, ri-raffinamento storico, **istanze generiche di sottodominio** (settimo compito), conferma identità, `CONTRADICTS` mancate, smistamento temporale. Scrive solo primitive esistenti (INGEST/PROMOTE + Famiglia B / `MEMBER_OF`). Ogni passata è loggata in `:JudgeRun`. Flag `ENABLE_JUDGE` (default true); `BACKBONE_COLLAPSE_THRESHOLD=0.90`. Il settimo compito è dietro `ENABLE_GENERIC_INSTANCES` (default **false**) e `GENERIC_INSTANCE_MIN_OBSERVATIONS=2`: un membro di catch-all senza genere figlio plausibile, osservato N volte consecutive come generico, viene ancorato a un'istanza generica del sottodominio (archi spostati, summary del generico mai sovrascritto, nodo originale tenuto con `merged_into`). Dalla Fase 18 il compito 5 (`FIND_MISSED_CONTRADICTIONS_CYPHER`) è scoped a `touched_ids` del batch; lista vuota/assente resta full-scan.
...
- Orchestrazione agentica (Fase 22): `backend/app/pipeline/context_agent.py` — ciclo ReAct su `call_structured` (niente tool-calling nativo OpenAI), cap `CONTEXT_AGENT_MAX_TURNS=4` (verificato in Fase 24, non ritoccato). Innesco = coda delle ipotesi promosse (medium/high) dopo `run_judge`, mai uno scan globale e mai su ogni chunk. Scritture solo via primitive esistenti (`write_node_relation` con testimoni, Famiglia B, `resolve_quantifier_scope` / `resolve_retraction_scope`). Timeout/errore/turni esauriti → fallback silenzioso, ipotesi `open`, `:AgentSearchRun` comunque scritto.
- Validazione del layer di contesto (Fase 24): corpus etichettato `tests/fixtures/context_layer_eval_corpus.py` (26 frammenti, classi a–d) e harness `evaluate_relevance_gate` / CLI [`backend/scripts/eval_context_layer.py`](./backend/scripts/eval_context_layer.py) (`--gate-only` | `--with-model-fallback` | `--count-calls`). Marcatori T2 con confine di parola (`contains_lexical_marker`: «finora è» non è più succession). Fallback a modello se il gate dà solo T1/`None` e una `:Relation` è stata scritta. Misurato sul corpus: gate-only **P=1.000 R=0.571 F1=0.727** (i 6 FN sono parafrasi senza marcatore); con fallback a stub **P=1.000 R=1.000 F1=1.000**. `PENDING_HYPOTHESIS_LISTEN_WINDOW=5` e `CONTEXT_AGENT_MAX_TURNS=4` confermati dai dati (non ritoccati). `ENABLE_CONTEXT_LAYER` resta **False**.
- Messa in produzione controllata (Fase 25): ispezione `GET /graph/context-layer/runs` (`:AgentSearchRun` + `:PendingHypothesis` aperte + contatori `:ContextLayerRun` per batch). Runbook a gradini in **Note operative**. Costo LLM extra documentato accanto alla tabella flag. Spegnere il flag lascia le ipotesi aperte inerti (mai `DELETE`). Nessun pannello frontend.
...
Policy di rollout: ogni flag resta spento finché la relativa suite di accettazione non è verde. Fasi 4–12 sono verdi salvo le faccette; `ENABLE_FACET_IDENTITY`, `ENABLE_CONTEXT_LAYER` e `ENABLE_GENERIC_INSTANCES` restano False.
...
| `ENABLE_CONTEXT_LAYER` | **False** | Fase 20–25 (spento → comportamento Fase 0–18 identico). Accensione solo a gradini: vedi Note operative. |
| `PENDING_HYPOTHESIS_LISTEN_WINDOW` | 5 | Fase 20/24: ascolto senza rinforzo; verificato sul corpus F24.1 (keep) |
| `CONTEXT_AGENT_MAX_TURNS` | 4 | Fase 22/24: cap turni ReAct; verificato sul corpus (keep; max pianificato 3) |
| `ENABLE_GENERIC_INSTANCES` | **False** | Fase 23: settimo compito del giudice (spento → gli altri sei invariati) |
| `GENERIC_INSTANCE_MIN_OBSERVATIONS` | 2 | Fase 23: passate consecutive `generico` prima del redirect |

**Chiamate LLM extra con il layer acceso** (F25.2; formula dal code path, non da un corpus di produzione — in `sm/` non c’è un corpus reale grande e una run live OpenAI non è un prerequisito):

- Fallback (`classify_fragment_relevance_with_model_fallback`): **+1** `call_structured` per chunk che ha scritto una `:Relation` e il cui gate deterministico è solo T1 o `None`. T2/T3 → **+0**.
- Agente: **fino a `CONTEXT_AGENT_MAX_TURNS` (4)** per ipotesi **promossa** (medium/high), dopo il giudice. Coda di promozione vuota → **+0**.
- Quantificatore / ritrattazione / create ipotesi: nessun LLM extra oltre il fallback (sono deterministici dato il segnale).

Esempi: worst case ≈ 1 fallback/chunk-con-nuova-relazione + 4/ipotesi-promossa; documento tipico senza marcatore T2 e una scrittura S0 → **+1**; documento con T2 esplicito e nessuna promozione → **+0**. Harness F24.2 sul corpus etichettato (non sul corpus reale del progetto): gate-only **P=1.000 R=0.571**; con fallback stub **P=1.000 R=1.000**. CLI: `python scripts/eval_context_layer.py --count-calls` (stima statica dal mix di classi, zero OpenAI).

### Debito noto

`merge_nodes` (percorso distruttivo) e il binario `RelationLabel.replaces` / `extends` restano finché `ENABLE_FACET_IDENTITY` e le transizioni temporali a tre vie non hanno una finestra di osservazione in produzione. Non rimossi in questa fase (F13.4 differito). Lo scan periodico sull'intera KB già ingerita resta il residuo #11: la Fase 18 ha solo ristretto il Compito 5 del giudice al batch corrente. Nessun pannello UI di storico (`node_history` è un helper, non un endpoint).
...
| 22 | Orchestrazione: il flusso agentico di verifica | completata |
| 23 | Istanze generiche per sottodominio | completata |
| 24 | Validazione e correzione del layer agentico di contesto | completata |
| 25 | Messa in produzione controllata del layer agentico di contesto | completata |
...
- **Eval layer di contesto (Fase 24)**: da `backend/`, `python scripts/eval_context_layer.py --gate-only` (zero OpenAI) oppure `--with-model-fallback` (stub a cue se manca la chiave; `--live-llm` per il modello reale). `--count-calls` stampa la stima F25.2 di chiamate extra dal mix di classi (zero OpenAI). Stampa precisione/recall misurati sul corpus F24.1.
- **Ispezione layer di contesto (F25.1)**: `GET /graph/context-layer/runs` (sola lettura, nessun pannello frontend). Restituisce `agent_runs` (`:AgentSearchRun`: turni, esito), `open_hypotheses` (`:PendingHypothesis` aperte) e `gate_runs` (`:ContextLayerRun` per job: T1/T2/T3/`model_fallback`, promotions, agent_runs, agent_turns_used). I contatori del gate stanno sul log node per batch, stesso schema di `:JudgeRun` — non un secondo endpoint `/stats`.
- **Attivazione a gradini di `ENABLE_CONTEXT_LAYER` (F25.4)** — procedura umana, non un job che si autopromuove. Il default resta **false**. Un solo flag copre gate + ipotesi + agente; non è stato aggiunto un secondo flag «solo gate». Equivalente pratico del passo (1):

  1. **Solo osservazione del gate, ambiente non di produzione.** Accendere `ENABLE_CONTEXT_LAYER=true` su un env non-prod. Ingerire un numero definito di documenti **isolati**: niente nomi propri che promuovono al primo frammento, niente secondo documento che rinforza la stessa `claim_target`. Non lanciare dreaming su quei job, oppure lanciarlo con coda di promozione vuota (zero `call_structured` dell’agente). Osservare `GET /graph/context-layer/runs` (`gate_runs.t1/t2/t3/model_fallback`, ipotesi `open` a `confidence=low`). Non alimentare documenti di rinforzo.
  2. **Verificare P/R.** Confrontare i falsi positivi osservati con i limiti F24 sul corpus etichettato: gate-only **P=1.000 R=0.571**; con fallback stub **P=1.000 R=1.000**. Se il gate sul testo reale introduce FP oltre quel recinto, non salire di gradino — la taratura su corpus di produzione resta fuori scope (residuo analogo al #4).
  3. **Catena completa su non-prod.** Ingerire anche documenti di rinforzo / testimoni nominati; far girare dreaming/giudice così l’agente consuma la coda. Controllare `agent_runs` (turni, `verdict`) e che le scritture S0 passino solo da `write_node_relation` con testimoni. Spegnere il flag e verificare che le `:PendingHypothesis` aperte restino nel grafo, inerti (nessuna `DELETE`, nessun S0 da ipotesi, nessun run agente).
  4. **Solo allora, produzione.** Accendere il flag in produzione dopo i tre passi sopra. Restare pronti a rimetterlo a `false`: lo spegnimento è sicuro (F25.3).
