...
- Event triage (gated, default off): `_task_event_triage` in `run_judge` (ultimo task) propone slot via `call_structured` e scrive solo tramite `validate_slot_proposal` / `apply_validated_slot`. Audit `:EventTriageRun` keyed per evento. Flag `ENABLE_EVENT_TRIAGE` default **false**, indipendente da `ENABLE_CONTEXT_LAYER` — a flag spento Fase 0–22 identico, zero query extra.
